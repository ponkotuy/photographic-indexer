...
case class PagingResponse[T](data: Seq[T], page: Int, perPage: Int, allCount: Long) {
  def withDateCounts(dateCounts: Seq[DateCount]) =
    PagingWithDateCountsResponse(data, page, perPage, allCount, dateCounts)
}
...
trait Pagination { self: ScalatraServlet =>
  def paging[T](dataQuery: Paging => Seq[T])(countQuery: => Long)(implicit encoder: Encoder[PagingResponse[T]]): PagingResponse[T] = {
    ...
    PagingResponse(data, page, perPage, allCount)
  }
}
